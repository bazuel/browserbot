import {BLCssRuleAddEvent, BLCssRuleRemoveEvent, BLEvent} from "browserbot-common";

export class CssDeserializer {

    mappedIndexes = new Map<HTMLStyleElement, { prevIndex: number, newIndex: number }[]>()
    ...
    deserialize(e: BLEvent, s: HTMLStyleElement) {
        if ('rule' in e) {
            ...
            this.mappedIndexes.get(s)!.push({prevIndex: ce.index, newIndex: newIndex!})
        } else {

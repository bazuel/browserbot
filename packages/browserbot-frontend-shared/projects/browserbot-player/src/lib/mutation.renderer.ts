...
export class MutationRenderer {
    ...
    render(e: BLMutationEventData, deserialized: Map<number, Node>, deserializer: ElementDeserializer, doc: Document, options: { debugging: boolean } = {debugging: false}) {
        try {
            const findElement = (id, event?) => {
                let el = deserialized.get(id) as HTMLElement
                if (!el && event)
                    el = deserializer.deserialize(event as DOMJsonElement, doc) as HTMLElement
                return el
            }

import {
    ...
    BLMutationEventData,
    BLScrollEvent, debounce,
    DOMJsonElement, RequestAnimationFrameTimer
} from "browserbot-common";
import {DomJsonMerger} from "../dom.json-merger";
...
import {MutationRenderer} from "../mutation.renderer";
...
export class PlayerComponent {
    ...
    private target!: { doc: Document; iframe: HTMLIFrameElement; readonly win: Window };
    ...
    constructor(container: HTMLDivElement, private options = {
    ...
    private applyFrameEvents(events: BLEvent[],) {
        let lastChecked = events.slice().reverse().find(e => e.name == "checked" || e.name == "input") as BLInputCheckboxChangeEvent
        let lastInput = events.slice().reverse().find(e => e.name == "value" || e.name == "input") as BLInputChangeEvent
        if (lastInput) {
            let el = this.deserializedMap.get(lastInput.target as unknown as number) as HTMLInputElement
            if (el && el.getAttribute) { // text and comment elements do not have getAttribute
        ...
        if (lastChecked) {
            let el = this.deserializedMap.get(lastChecked.target as unknown as number) as HTMLInputElement
            if (el) {
        ...
        let scrolls = {}
        events.filter(e => e.name == "scroll" || e.name == "elementscroll")
            .map(e => e as BLScrollEvent).forEach(e => scrolls[e.target as unknown as string] = e)
    ...
    private applyCssEvents(events: BLEvent[]) {
        let cssEvents = events.filter(e => e.name == "css-add" || e.name == "css-remove") as BLCssRuleRemoveEvent[]
        cssEvents.sort((c1, c2) => c1.index - c2.index)
        for (let e of cssEvents) {
            let style = this.target.doc.querySelector("#bl_style_" + (e as BLCssRuleRemoveEvent).target) as HTMLStyleElement
            if (!style) {

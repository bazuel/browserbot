...
export class DomMonitor implements BLMonitor {
    ...
    enable() {
        ...
        this.mutationObserver = new DomFrameMutationsMonitor((frameMutations) => {
            let mutations = new MutationSerializer(this.elementsMap, elid, serializer).serialize(frameMutations)
            let e: BLDomEvent = {mutations, timestamp: new Date().getTime(), type: 'dom', name: "dom-change"}
